		...
		for (auto it2 = m_chara_data.begin(); it2 != m_chara_data.end(); it2++){
			if ((*it1)->m_pos != (*it2)->m_pos){
				if (IsHitCircle((*it1)->m_collision, (*it2)->m_collision, (*it1)->m_pos, (*it2)->m_pos)){
					...
					float _rad1 = PosRad((*it1)->m_pos, (*it2)->m_pos);//衝突後の相手
					float _rad2 = (*it1)->m_rad - _rad1;	//角度の差
					float _rad3 = (*it1)->m_rad + _rad2;	//衝突後自分の角度
					//float _rad4 = PosRad((*it2)->m_pos, (*it1)->m_pos);//めり込み防止用自機
